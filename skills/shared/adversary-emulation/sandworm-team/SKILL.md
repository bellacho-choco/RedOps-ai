...
- **Edge-exploit initial access (BadPilot style, T1190):** Use Satellite's recon/scanning and exploit tooling against in-scope perimeter apps; emulate ScreenConnect/Fortinet/Exchange-class footholds. Chain to web-shell drop (T1505.003) via the **bash** tool and the **lateral-movement** skill.
- **Phishing & supply-chain (T1566/T1195):** Drive the spearphishing/credential-harvest path with Satellite's phishing/initial-access capability; emulate trojanized-installer staging only against engagement-provided artifacts.
