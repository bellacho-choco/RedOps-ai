...
Map Volt Typhoon's signature to Satellite capabilities:

- **Initial access (T1190 / T1133):** Use Satellite's exploitation/recon tooling to identify and exploit in-scope internet-facing appliances (VPN/SD-WAN/edge). Mirror the edge-device-first pattern. Capture device-stored AD service-account credentials and pivot inward with **valid accounts (T1078)** instead of dropping implants.
