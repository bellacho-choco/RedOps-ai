---
name: apt33-elfin
description: "Adversary-emulation profile for APT33 (Elfin, Peach Sandstorm, HOLMIUM), a suspected Iranian state-sponsored espionage group, mapped to MITRE ATT&CK G0064 with Satellite emulation guidance."
...
Map APT33's signature chain to Satellite's own capabilities:

- **Initial access — password spray (`T1110.003`) + cloud valid accounts (`T1078.004`):** This is the modern APT33 signature. Use Satellite's cloud skills to run a low-and-slow spray against the engagement's M365/Entra ID tenant with a custom user-agent and lockout-aware throttling, mirroring the `go-http-client` low-volume pattern. Pair with the engagement's authorized credential list. Avoid lockouts to stay in scope.
...
- **Execution / staging (`T1059.001`, `T1105`):** Use Satellite's bash/PowerShell tooling to fetch and run a stager from your redirector, mirroring PowerShell download-and-execute.
...
- **Cloud/AD discovery (`AzureHound`/`Roadtools`/AD snapshot):** Use Satellite's cloud and AD skills to enumerate Entra ID via Graph/Azure REST (AzureHound-style) and pull an AD snapshot/BloodHound collection on-prem — the exact recon APT33 runs post-auth.
...
- **Lateral movement (`T1078`, SMB):** Use Satellite's lateral-movement skill for SMB-based propagation with recovered valid accounts; optionally emulate AnyDesk RMM deployment if RMM abuse is in scope.
