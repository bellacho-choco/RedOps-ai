...
description: Operate BloodHound Community Edition v9.2.2 via Satellite's bhce_* tools — health check, Cypher passthrough, SharpHound ZIP ingest. Replaces the in-house ingest + ESC* post-process pipeline per ADR-0005.
metadata:
...
# BloodHound CE via Satellite's `bhce_*` Tools

Satellite ships a sidecar BloodHound Community Edition v9.2.2 stack
...
The Satellite KGStore still owns web, cloud, and smart-contract
...
4. **Cross-domain plays** — when an AD path terminates and the chain
   needs to cross into the Satellite KGStore (web exploitation, cloud
